...
It reuses several projects from this monorepo as-is (nothing reimplemented):
the Port Scanner engine (`packages/port-scanner`), the StegKit library
(`packages/stegkit`), the Asphalt packet capture/analysis engine
(`packages/asphalt`), and the Image-Geolocator web app (`apps/osint`).
...
- **INTEL** — analysis of files, media and images:
  ...
  - *OSINT Geo* — starts the local `osint` Next.js dev server and embeds it
    ...
    `VISION_API_KEY` in `apps/osint/.env.local`).
...
cd apps/overlay && python main.py
```
...
Overlay/
  ...
  overlay/
    ...
    core/
      bridge.py               imports the engines in packages/; PROJECT_ROOT
...
here. `overlay/core/bridge.py` puts the `packages/` engines on `sys.path` and
