...
class ChatGPTPanel(QWidget):
    def __init__(self, parent=None):
        ...
        # The browser (a Chromium process, persistent profile and the chatgpt.com
        # page load) is created on first show, not when the bar starts up.
        self._root = root
        self.profile = None
        self.view = None
        self._popups = []
    ...
    def _ensure_view(self):
        """Build the web view if it doesn't exist yet. Returns it (None without WebEngine)."""
        if self.view is not None or not WEBENGINE_OK:
            return self.view
        ...
        self._root.addWidget(self.view, 1)
        return self.view
    ...
    def _load(self, text):
        if not text or not self._ensure_view():
            return
        if "://" not in text:
            text = "https://" + text
