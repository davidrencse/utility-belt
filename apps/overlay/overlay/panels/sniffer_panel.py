...
engine in packages/asphalt/src. It reuses that project's pieces verbatim (the
...
class SnifferPanel(QWidget):
    ...
    def _build(self):
        ...
        # Importing Asphalt/scapy takes about a second, so it happens in
        # InterfaceWorker; failures arrive via _on_interfaces_failed.
        if not eng.SNIFF_DIR:
            self.start_btn.setEnabled(False)
            self.status.setText("Asphalt engine not found in packages/asphalt")
        else:
            self.status.setText("loading interfaces...")
    ...
    def _load_interfaces(self):
        ...
        self.ifaces.failed.connect(self._on_interfaces_failed)
        self.ifaces.start()

    def _on_interfaces_failed(self, msg):
        self.iface.clear()
        self.iface.addItem("unavailable", None)
        self.start_btn.setEnabled(False)
    ...
    def _on_batch(self, rows, stats):
        ...
        # to see what just happened. Rows beyond _MAX_ROWS would be trimmed
        # straight away, so don't insert them; insert the rest as one block.
        rows = rows[-_MAX_ROWS:]
        table = self.packets
        table.setUpdatesEnabled(False)
        try:
            table.model().insertRows(0, len(rows))
            ...
            if table.rowCount() > _MAX_ROWS:
                table.setRowCount(_MAX_ROWS)
        finally:
    ...
    def _set_view(self, view):
        ...
        idx = 0 if view == "live" else 1
        if self.stack.currentIndex() != idx:
            self.stack.setCurrentIndex(idx)
            motion.reveal(self.stack.currentWidget(), dx=14 if idx else -14, dy=0)
        if self.worker and self.worker.isRunning():
