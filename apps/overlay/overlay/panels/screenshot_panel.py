...
class ScreenshotPanel(QWidget):
    def __init__(self, take=None, send_to_chat=None, parent=None):
        ...
        # The grid decodes a thumbnail per saved shot (up to screenshot_keep), so it
        # is filled on first show and only kept live while visible.
        self._dirty = True
        history.added.connect(self._on_added)
        history.changed.connect(self._on_changed)

    def showEvent(self, e):
        super().showEvent(e)
        if self._dirty:
            self._rebuild()
            if getattr(self, "_select_newest", False) and self.grid.count():
    ...
    def _on_added(self, path):
        if self._dirty or not self.isVisible():
            self._dirty = True
    ...
    def _sync_actions(self):
        ...
        motion.appear(self.grid, n > 0)
        motion.appear(self.empty, n == 0)
