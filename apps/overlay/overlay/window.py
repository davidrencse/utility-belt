...
class TabBar(QWidget):
    ...
    def __init__(self, names, on_select, on_detach=None, parent=None):
        ...
        self._torn = False
        # animated indicator: a spring over (x, w) keeps its velocity when
        # retargeted, so rapid tab cycling glides instead of restarting
        self._ind_x = 0.0
        self._ind_w = 0.0
        self._ind = motion.Spring((0.0, 0.0), T.SPRING_SNAPPY, owner=self,
                                  rest=0.2, on_change=self._on_anim)
    ...
    def set_active(self, idx):
        ...
        # first placement snaps; later changes spring
        self._ind.set((tx, tw), immediate=self._ind_w == 0.0)
        self.update()

    def _on_anim(self, v):
        self._ind_x, self._ind_w = v
        self.update()
...
class OverlayWindow(QWidget):
    ...
    def _select(self, idx):
        ...
        if panel in self._detached:          # torn-off: raise its own window
            ...
            motion.fade_window(self, 1.0)     # main isn't showing the browser
            return
        changed = self.stack.currentWidget() is not panel
        self.stack.setCurrentWidget(panel)
        if changed:
            self._fade(panel, 1 if idx >= prev else -1)
    ...
    def _apply_window_opacity(self):
        ...
        motion.fade_window(self, self._rest_opacity())

    def _fade(self, panel, direction=1):
        ...
        if panel is self.chat or panel is self.intel:
            return
        # same spring/tween system as the sub-tabs: slide in from the side
        # you navigated toward; interrupting mid-flight carries on smoothly
        motion.reveal(panel, dx=16 * direction, dy=0)
    ...
    def _rest_opacity(self):
        ...
        return web_opacity_frac() if chat_shown else 1.0

    def toggle_visible(self):
        if self.isVisible() and not motion.is_leaving(self):
            motion.dismiss(self, offset=(0, -8))
        else:
            motion.present(self, self.pos(), offset=(0, -8),
                           opacity=self._rest_opacity())
            self.raise_()
...
class DetachedWindow(QWidget):
    ...
    def _on_setting_opacity(self, key, _value):
        if key == "card_opacity" and self._is_web:
            motion.fade_window(self, web_opacity_frac())
