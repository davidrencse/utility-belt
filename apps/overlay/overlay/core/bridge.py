"""
Imports the existing Port Scanner engine (packages/port-scanner) without
copying or rewriting a line of it. Everything in it is standard-library
...
Same trick for two more packages: StegKit (packages/stegkit) and
Asphalt (packages/asphalt), the packet capture/decode/analysis toolkit.
"""
import os
import platform
import subprocess
...
# overlay/core -> overlay -> apps/overlay -> apps -> repo root
...
_ENGINE_DIR = os.path.normpath(os.path.join(PACKAGES_DIR, "port-scanner"))
...
# --- StegKit steganography (packages/stegkit, imported like the scanner) ---
_STEGO_DIR = os.path.normpath(os.path.join(PACKAGES_DIR, "stegkit"))
if os.path.isdir(_STEGO_DIR) and _STEGO_DIR not in sys.path:
    sys.path.insert(0, _STEGO_DIR)

STEGO_DIR = _STEGO_DIR
_STEGO_NAMES = ("STEGO_OK", "STEGO_ERROR", "stego_image", "stego_text", "stego_errors")


def _load_stego():
    # stegkit pulls in numpy, Pillow and cryptography (hundreds of ms); import it
    # the first time the Steganography tab touches it rather than at app start.
    g = globals()
    try:
        from stegkit import image as stego_image
        from stegkit import text as stego_text
        from stegkit import errors as stego_errors
        g.update(STEGO_OK=True, STEGO_ERROR=None, stego_image=stego_image,
                 stego_text=stego_text, stego_errors=stego_errors)
    except Exception as exc:        # pragma: no cover - surfaced in the UI
        g.update(STEGO_OK=False, STEGO_ERROR=f"{type(exc).__name__}: {exc}",
                 stego_image=None, stego_text=None, stego_errors=None)


def __getattr__(name):
    if name in _STEGO_NAMES:
        _load_stego()
        return globals()[name]
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


# --- Asphalt packet capture / analysis (packages/asphalt, imported lazily) -
...
_SNIFF_DIR = os.path.normpath(os.path.join(PACKAGES_DIR, "asphalt", "src"))
