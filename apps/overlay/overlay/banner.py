...
from PySide6.QtGui import (QPainter, QColor, QBrush, QPainterPath, QPen,
                           QCursor, QFont, QLinearGradient, QPixmap)
...
class Dropdown(QWidget):
    ...
    def paintEvent(self, e):
        # Children (gauges, sparklines) repaint many times a second and, on a
        # translucent window, every child update repaints this background too.
        # Render the shadow/card/border once per size+opacity and blit it.
        bg = self._background()
        p = QPainter(self)
    ...
    def _background(self):
        ...
        key = (self.width(), self.height(), int(settings.get("card_opacity")), dpr)
