 ...
 */
// Ground elevation never changes; remember it per ~1 m coordinate so repeat
// uploads of the same place skip the request (and its latency) entirely.
const elevationCache = new Map<string, number>();
const ELEVATION_CACHE_MAX = 5000;
const elevationKey = (p: GeoPoint) => `${p.lat.toFixed(5)},${p.lon.toFixed(5)}`;

export async function attachElevation(points: GeoPoint[], signal?: AbortSignal): Promise<void> {
  if (!points.length) return;
  const missing: GeoPoint[] = [];
  for (const p of points) {
    const cached = elevationCache.get(elevationKey(p));
    if (cached !== undefined) p.elevationM = cached;
    else missing.push(p);
  }
  if (!missing.length) return;
  const lat = missing.map((p) => p.lat.toFixed(5)).join(",");
  const lon = missing.map((p) => p.lon.toFixed(5)).join(",");
  try {
    ...
    missing.forEach((p, i) => {
      if (typeof j.elevation![i] !== "number") return;
      p.elevationM = Math.round(j.elevation![i]);
      if (elevationCache.size >= ELEVATION_CACHE_MAX) {
        elevationCache.delete(elevationCache.keys().next().value!);
      }
      elevationCache.set(elevationKey(p), p.elevationM);
    });
  } catch {
