...
// In-flight lookups by the same key. The result cache is only filled once a
// request finishes, so without this two concurrent identical queries (common:
// geovision and a sign both name the same landmark) would both queue for the
// 1-req/sec slot and both hit the network.
const inflight = new Map<string, Promise<NominatimHit | null | undefined>>();
...
/**
 * One Nominatim search, serialized through the politeness limiter. Resolves to
 * the chosen hit, null for "no match" (cacheable), or undefined when the request
 * itself failed (not cached).
 */
async function lookup(
  ...
  // serialize the actual request (politeness)
  const res = await geocodeLimit(() =>
    safeFetch(url, { headers: { accept: "application/json" } }, 9000, signal),
  );
...
const geocodeSource: Source = {
  ...
  async run(e, ctx) {
    ...
    if (hit === undefined) {
      ...
      if (!pending) {
        pending = lookup(e.value, near, ctx.signal).then((h) => {
          if (h !== undefined) cacheSet(key, h); // undefined = request failed; retry next time
          return h;
        });
