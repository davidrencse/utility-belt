...
export interface Source {
  ...
  requiresKey?: string;
  /**
   * The source throttles its own network calls (e.g. geocode's 1-req/sec queue),
   * so the engine runs it outside the global concurrency limiter. Otherwise jobs
   * merely WAITING in that private queue would hold global slots and starve
   * unrelated work such as vision calls for the next image.
   */
  selfLimited?: boolean;
