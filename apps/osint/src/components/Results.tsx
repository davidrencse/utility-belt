...
function AnalysisView({ result, images }: { result: GeolocateResult; images?: Record<string, string> }) {
  ...
  // one pass over findings instead of a filter per image on every render
  const byImage = useMemo(() => {
    const m = new Map<string, Finding[]>();
    for (const f of result.findings) {
      if (!f.image) continue;
      const list = m.get(f.image);
      if (list) list.push(f);
      else m.set(f.image, [f]);
    }
    return m;
  }, [result.findings]);
    ...
    <div className="space-y-12">
      {names.map((name) => (
        <ImageAnalysis
          ...
          name={name}
          src={images?.[name]}
          findings={byImage.get(name) ?? NO_FINDINGS}
        />
      ))}
    </div>
  );
...
export function Results({
  ...
  // Stable reference: MapView rebuilds the whole map whenever `points` changes identity.
  const geo = result.geo ?? NO_POINTS;
  ...
  return (
    <div className="mt-10 rise">
      ...
      {/* Kept mounted while hidden: unmounting destroys the WebGL map, and coming
          back would re-create it and refetch style + tiles. */}
      <div hidden={tab !== "map"}>
        <MapView points={geo} active={tab === "map"} />
      </div>
