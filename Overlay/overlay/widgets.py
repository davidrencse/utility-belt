...
class Sparkline(QWidget):
    ...
    def paintEvent(self, _e):
        ...
        big = r.height() > 90
        lab = QFont(T.MONO, 8 if big else 7, QFont.Bold)
        lab.setLetterSpacing(QFont.AbsoluteSpacing, 1.5)
        val = QFont(T.MONO, 11 if big else 9, QFont.DemiBold)
        p.setFont(lab)
        p.setPen(T.TEXT_DIM)
...
class Gauge(QWidget):
    ...
    def paintEvent(self, _e):
        ...
        fnum = QFont(T.UI, num_pt, QFont.DemiBold)
        fpct = QFont(T.UI, max(7, int(num_pt * 0.5)), QFont.DemiBold)
        num = f"{shown:.0f}"
        from PySide6.QtGui import QFontMetricsF
        wn = QFontMetricsF(fnum).horizontalAdvance(num)
        wp = QFontMetricsF(fpct).horizontalAdvance("%")
        cx = rect.center().x() - (wn + wp) / 2
        base_y = rect.center().y() + QFontMetricsF(fnum).capHeight() / 2 - (
        ...
        ftitle = QFont(T.MONO, 7, QFont.Bold)
        ftitle.setLetterSpacing(QFont.AbsoluteSpacing, 1.8)
        p.setPen(T.TEXT_MUTED)
        p.setFont(ftitle)
