...
class Sampler(QThread):
    ...
    def run(self):
        ...
        self._sess0 = self._last_net           # totals at app start
        i = 0
        while self._running:
            if self._paused:
                self.msleep(200)
            ...
            if mem:
                _rt, _rf, st, sf = mem
                s.swap_total_gb = round(st / 1024**3, 1)
                s.swap_used_gb = round((st - sf) / 1024**3, 1)
            ...
            s.sess_down = max(0, cur[0] - self._sess0[0])
            s.sess_up = max(0, cur[1] - self._sess0[1])
