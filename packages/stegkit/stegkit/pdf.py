...
from .core import bits_to_bytes, pack, unpack
...
def decode(path, password=None, technique="auto") -> bytes:
    ...
    if technique in {"auto", "whitespace"} and COMMENT_PREFIX in raw:
        ...
        raw_bits = np.frombuffer(encoded, dtype=np.uint8)
        raw_bits = raw_bits[(raw_bits == 9) | (raw_bits == 32)]
        frame = bits_to_bytes(raw_bits == 9)
