...
from .core import HEADER, bits_to_bytes, frame_length_from_prefix, pack, unpack
...
def encode(source, output, data: bytes, password: str | None = None) -> None:
    ...
    bits = np.unpackbits(np.frombuffer(pack(data, password), dtype=np.uint8)).astype(np.int32)
