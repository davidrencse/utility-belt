        ...
        with self._lock:
            ...
            def packet_callback(packet):
                ...
                try:
                    ...
                    # Use the captured wire bytes when available; bytes(packet)/len(packet)
                    # re-serialise the whole layer tree each time.
                    raw = getattr(packet, 'original', None) or bytes(packet)
                    raw_len = len(raw)
                    packet_data = {
                        'ts': packet.time,
                        'data': raw,
                        'wirelen': getattr(packet, 'wirelen', None) or raw_len,
                    }

                    try:
                        packet_queue.put_nowait(packet_data)
                        stats['packets_since_update'] += 1
                        stats['bytes_since_update'] += raw_len
                        
                        # DEBUG: First few packets
                        if stats['packets_total'] + stats['packets_since_update'] < 5:
                            print(f"DEBUG: Captured packet {stats['packets_total'] + stats['packets_since_update']}: {raw_len} bytes")
