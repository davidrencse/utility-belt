...
def decode_packet(raw: RawPacket) -> DecodedPacket:
    ...
    if dst_ip:
        ip_multicast, ip_broadcast = _ip_dst_class(dst_ip)
        if ip_multicast:
            is_multicast = True
        if ip_broadcast:
            is_broadcast = True
...
def _parse_l4(data: bytes, offset: int, ip_protocol: int) -> Tuple[Optional[str], Optional[int], Optional[int], Optional[int], Optional[int], Optional[int], Optional[int], Optional[int], Optional[int], Optional[int], DecodeQuality]:
    ...
    if ip_protocol == IP_PROTO_TCP:
        ...
        src_port, dst_port, seq, ack = _TCP_PORTS_SEQ_ACK.unpack_from(data, offset)
        ...
        window = _U16.unpack_from(data, offset + 14)[0]
        mss = None
        if data_offset > 20:
...
def _ip_dst_class(dst_ip: str) -> Tuple[bool, bool]:
    """(is_multicast, is_broadcast) for a destination IP string; cached per address."""
    try:
        ip_obj = ipaddress.ip_address(dst_ip)
    except Exception:
        return False, False
    return bool(ip_obj.is_multicast), ip_obj.version == 4 and dst_ip == "255.255.255.255"
...
def _format_ipv6(addr: bytes) -> Optional[str]:
    if len(addr) != 16:
        return None
    return _format_ipv6_cached(bytes(addr))


@lru_cache(maxsize=65536)
def _format_ipv6_cached(addr: bytes) -> Optional[str]:
    try:
