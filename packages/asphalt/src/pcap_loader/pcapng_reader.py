...
class PcapngReader(IPacketSource):
    ...
    def _parse_interface_description_block(self, offset: int):
        ...
        while True:
            ...
            if opt_code == 2:  # if_name (UTF-8 string)
                ...
                interface_info['tsresol_binary'] = bool(tsresol_byte & 0x80)
            # Pad to 32-bit
    ...
    def _parse_enhanced_packet_block(self, offset: int, packet_id: int = 0) -> RawPacket:
        """
        ...
        # Binary resolutions (2**-n) must be checked first: n == 6 or 9 is not μs/ns.
        if if_info.get('tsresol_binary'):
            timestamp_us = (ts_raw * 1_000_000) >> tsresol
        elif tsresol == 6:  # μs
            timestamp_us = ts_raw
        elif tsresol == 9:  # ns → μs
            timestamp_us = ts_raw // 1000
        elif tsresol > 6:
            timestamp_us = ts_raw // (10 ** (tsresol - 6))
        else:
            timestamp_us = ts_raw * (10 ** (6 - tsresol))
    ...
    def __iter__(self) -> Iterator[RawPacket]:
        ...
        while offset < len(self.mmap):
            ...
            if block_type == self.BLOCK_TYPE_SECTION_HEADER:
                ...
                try:
                    packet = self._parse_enhanced_packet_block(offset, packet_id)
                    ...
                    # Report the first few only: a systematic failure would otherwise print per packet.
                    failures += 1
                    if failures <= 5:
