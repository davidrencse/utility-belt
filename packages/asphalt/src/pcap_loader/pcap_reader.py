...
class PcapReader(IPacketSource):
    ...
    def __iter__(self) -> Iterator[RawPacket]:
        ...
        mm = self.mmap
        file_len = len(mm)
        unpack_header = struct.Struct(self.byte_order + 'IIII').unpack_from
        is_ns = self.is_nanosecond
        link_type = self.link_type
        while offset + 16 <= file_len:
            packet_start = offset
            # ts_sec, ts_frac (micro/nanoseconds), caplen (bytes saved), wirelen (original size)
            ts_sec, ts_frac, caplen, wirelen = unpack_header(mm, offset)
            if is_ns:
            ...
            elif timestamp_us < self._time_range[0] or timestamp_us > self._time_range[1]:
                start, end = self._time_range
                self._time_range = (min(start, timestamp_us), max(end, timestamp_us))

            # Classic pcap records are NOT padded: the next header follows caplen bytes directly.
            offset += caplen
