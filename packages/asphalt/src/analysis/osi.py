...
def add_tag_counts(summary: dict, tags: Iterable[str]) -> None:
    """Accumulate one tag list into a summary built by empty_tag_summary()."""
    for tag in tags:
...
def count_tags(tag_lists: Iterable[Iterable[str]]) -> dict:
    summary = empty_tag_summary()
    for tags in tag_lists:
        add_tag_counts(summary, tags)
    return summary
