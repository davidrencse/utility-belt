...
def _get_local_ips():
    """This host's IPs (hostname, interfaces, routes). Queried once: the scapy interface
    and route lookups are slow on Windows and the answer does not change per capture."""
    global _LOCAL_IPS
    with _LOCAL_IPS_LOCK:
        ...
        try:
            ...
            pass
        _LOCAL_IPS = frozenset(ip for ip in local_ips if ip and ip not in ("0.0.0.0", "127.0.0.1"))
        return _LOCAL_IPS


@functools.lru_cache(maxsize=65536)
def _is_non_public_ip(ip):
    try:
...
class _ShowEventFilter(QtCore.QObject):
    """Calls callback(widget) whenever a watched widget is shown."""
...
class PacketTableModel(QtCore.QAbstractTableModel):
    """Read-only view over packet dicts; cells are formatted only when Qt paints them."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._packets = []
        self._columns = ["packet"]
    ...
    def set_packets(self, packets):
        ...
        columns = [k for k in _PACKET_TABLE_PREFERRED if k in keys]
        columns += [k for k in sorted(keys) if k not in columns]
        self.beginResetModel()
        self._packets = packets
        self._columns = columns or ["packet"]
        self.endResetModel()
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if not isinstance(pkt, dict):
            return str(pkt) if index.column() == 0 else None
        val = pkt.get(self._columns[index.column()])
...
class RowsTableModel(QtCore.QAbstractTableModel):
    """Read-only table of dict rows (keyed by column) or sequence rows; formatted on paint."""
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if isinstance(row, dict):
            return str(row.get(self._columns[col], ""))
        if isinstance(row, (list, tuple)):
            return str(row[col]) if col < len(row) else ""
        return str(row) if col == 0 else ""
...
def _make_rows_table(columns, min_rows):
    table = QtWidgets.QTableView()
    table.setModel(RowsTableModel(columns, table))
...
class AsphaltApp(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self._json_show_filter = _ShowEventFilter(self._render_pending_json, self)
    ...
    def _build_ui(self):
        ...
        # ResizeToContents re-measures every row on each layout pass; size columns once after
        # the first fill instead (see refresh_packet_overview).
        header.setSectionResizeMode(QtWidgets.QHeaderView.Interactive)
        header.setSectionResizeMode(2, QtWidgets.QHeaderView.Stretch)
        header.setSectionResizeMode(3, QtWidgets.QHeaderView.Stretch)
        self.packet_overview.setUniformRowHeights(True)
        self.packet_overview.setTextElideMode(QtCore.Qt.ElideNone)
        self._overview_packets = []
        self._overview_row_cache = {}
        self._overview_row_cache_owner = None
    ...
    def _build_capture_panel(self):
        ...
        # Interface discovery imports scapy and queries Windows adapters (seconds); fill it in
        # from a background thread so the window appears immediately.
        self.interface_edit = QtWidgets.QLineEdit("")
        threading.Thread(target=self._discover_default_iface, daemon=True).start()
        ...
        # Filtering rebuilds both packet views; wait for a pause in typing instead of every keystroke.
        self._filter_timer = QtCore.QTimer(self)
        self._filter_timer.setSingleShot(True)
        self._filter_timer.setInterval(250)
        self._filter_timer.timeout.connect(self.apply_filter)
        self.filter_edit.textChanged.connect(self._filter_timer.start)
    ...
    def _discover_default_iface(self):
        try:
            iface = get_default_scapy_iface()
        except Exception:
            iface = ""
        _get_local_ips()  # warm the cache used by refresh_top_domains

        def apply():
            if iface and not self.interface_edit.text().strip():
                self.interface_edit.setText(iface)
        self.ui_call.emit(apply)

    def _resolve_scapy_interface(self, requested):
        if (requested or "").strip().startswith(r"\Device\NPF_"):
            return requested.strip()  # already a concrete adapter; skip the slow enumeration
        try:
            from capture.scapy_backend import ScapyBackend
        except Exception:
            return requested
    ...
    def _make_kv_table(self, min_rows=10):
        return _make_rows_table(["Key", "Value"], min_rows)

    def _make_table(self, columns, min_rows=10):
        return _make_rows_table(columns, min_rows)

    def _set_kv_rows(self, table, rows):
        table.model().set_rows([(row[0], row[1]) for row in rows])
    ...
    def _set_dynamic_rows(self, table, items, columns=None):
        ...
        # Non-dict items were always shown whole in the first column.
        items = [item if isinstance(item, dict) else (str(item),) for item in items]
        table.model().set_rows(items, columns)

    def _set_json_text(self, editor, obj):
        # Pretty-printing large analyzer output is slow and these panels usually sit on hidden
        # tabs, so defer the work until the editor is actually shown.
        editor._pending_json = obj
        if editor.isVisible():
            self._render_pending_json(editor)
        elif not getattr(editor, "_json_filter_installed", False):
            editor.installEventFilter(self._json_show_filter)
            editor._json_filter_installed = True

    def _render_pending_json(self, editor):
        if not hasattr(editor, "_pending_json"):
            return
        obj = editor.__dict__.pop("_pending_json")
        try:
    ...
    def _build_packet_tab(self):
        ...
        self.packet_table_model = PacketTableModel(self.packet_table)
        self.packet_table.setModel(self.packet_table_model)
        self.packet_table.horizontalHeader().setStretchLastSection(True)
        self.packet_table.horizontalHeader().setDefaultSectionSize(120)
    ...
    def refresh_packets(self, packets=None):
        ...
        self.packet_table_model.set_packets(packets)
    ...
    def apply_filter(self):
        ...
        packets = [packet for packet, hay in zip(self.latest_packets, self._packet_search_cache) if term in hay]
        self.refresh_packets(packets)
    ...
    def refresh_packet_overview(self, packets=None):
        ...
        self.packet_overview.setUpdatesEnabled(False)
        ...
        if self._overview_row_cache_owner is not self.latest_packets:
            self._overview_row_cache = {}
            self._overview_row_cache_owner = self.latest_packets
        row_cache = self._overview_row_cache
        # Items carry an index into this list; storing the dict itself via setData converts it
        # to a QVariant per row, which dominated the rebuild cost.
        self._overview_packets = overview_packets = []
        show_indicator = QtWidgets.QTreeWidgetItem.ShowIndicator
        ...
        try:
            for group_name, group_packets in groups:
                ...
                for pkt in group_packets:
                    ...
                    if row is None:
                        row = row_cache[key] = self._packet_row(pkt)
                    item = QtWidgets.QTreeWidgetItem(row)
                    item.setChildIndicatorPolicy(show_indicator)
                    item.setData(0, user_role, len(overview_packets))
                    overview_packets.append(pkt)
            ...
            self.packet_overview.setUpdatesEnabled(True)
    ...
    def _ensure_packet_details(self, item):
        ...
        if not isinstance(idx, int) or not 0 <= idx < len(self._overview_packets):
            return
        pkt = self._overview_packets[idx]
        if not isinstance(pkt, dict):
            return
